...
static void melee_attack(World *world, Entity *source) {
	...
	WeaponDataMelee *wd = get_data_melee(int id);
...
static void create_projectile(World *world, Entity *source, int projectile_id) {
	...
	p->x = source->x + source->whidth/2; p->y = source->y + source->heigth/2;
	...
	if (data->angle == 0) {
		...
		p->speed_x = (data->speed) * (float) cos(PI*(data->angle)/180.0);
		p->speed_y = (data->speed) * (float) sin(PI*(data->angle)/180.0);
		p->type_in_hand = 0;
	}
	if (!source->is_facing_right) {
		p->speed_x = -p->speed_x;
...
#define DOWNWARD_MAX_SPEED		-1//TODO
		...
		self -> x += (self->speed_x)*delta; self -> y += (self->speed_x)*delta;
		self -> speed_y -= DOWNWARD_ACCELLERATION*delta;
