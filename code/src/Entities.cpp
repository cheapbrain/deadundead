...
int createPlayer(World *world, float x, float y, float speedX, float speedY, float mass, char* name){
	...
	world->mask[entity] = COMPONENT_POSITION | COMPONENT_VELOCITY | COMPONENT_APPEARANCE | COMPONENT_MASS | COMPONENT_COLLISION | COMPONENT_PLAYER;
	...
	world->massComponents[entity].mass = mass;
